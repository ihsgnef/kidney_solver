...
if __name__ == '__main__':
    ...
    digraph_edges = []
    for line in digraph_lines[1 : n_digraph_edges + 1]:
        tokens = [x for x in line.split()]
        source = int(tokens[0])
        target = int(tokens[1])
        score = float(tokens[2])
        digraph_edges.append((source, target, score))
    graph = DynamicKidneyGraph(digraph_edges)

    if len(input_lines) > len(digraph_lines):
        ndd_lines = input_lines[n_digraph_edges + 2:]
        altruists = kidney_ndds.read_ndds(ndd_lines, graph.digraph)
    else:
        altruists = []

    start_time = time.time()
    cfg = kidney_ip.OptConfig(graph.digraph, altruists, args.cycle_cap, args.chain_cap, args.verbose,
